...
impl CombinatorialMap {
    ...
    /// A **presentation change**: renumber edges by `edge_perm` and optionally swap the two darts
    /// of edge `i` when `flip[i]`. This is an isomorphism of maps — it commutes with α by
    /// construction — and models a receiver who samples the same drawing with different bookkeeping.
    ///
    /// One generator of the reading class 𝔽₀ (spec/reading-invariance-v1.md).
    pub fn relabel(&self, edge_perm: &[usize], flip: &[bool]) -> CombinatorialMap {
        let e = self.edge_count();
        assert_eq!(edge_perm.len(), e, "edge_perm must cover the edges");
        assert_eq!(flip.len(), e, "flip must cover the edges");
        let f = |d: Dart| -> Dart {
            let (i, s) = (d / 2, d % 2);
            let s2 = if flip[i] { 1 - s } else { s };
            2 * edge_perm[i] + s2
        };
        let mut sigma = vec![0; self.n_darts];
        let mut dart_origin = vec![String::new(); self.n_darts];
        for d in 0..self.n_darts {
            sigma[f(d)] = f(self.sigma[d]);
            dart_origin[f(d)] = self.dart_origin[d].clone();
        }
        CombinatorialMap {
            sigma,
            dart_origin,
    ...
    /// The invariants the reading-invariance program tracks: components, face count, genus, and
    /// the multiset of degrees ≠ 2. Each is stable under all three 𝔽₀ generators — relabeling
    /// (isomorphism), mirror (orbit reversal), and subdivision (adds only degree-2 vertices).
    pub fn essential_invariants(&self) -> (usize, usize, Option<u32>, Vec<usize>) {
        (
            self.components(),
            self.faces().len(),
            self.genus(),
...
mod tests {
    ...
    // --- reading invariance, the 𝔽₀ floor (spec/reading-invariance-v1.md) ---

    /// Deterministic pseudo-random stream — no dependencies, reproducible failures.
    struct Lcg(u64);
    impl Lcg {
    ...
    /// The 𝔽₀ battery for a map: mirror, relabelings, subdivisions, and their composite.
    fn f0_battery(m: &CombinatorialMap, seed: u64) -> Vec<CombinatorialMap> {
        let mut rng = Lcg(seed);
        let e = m.edge_count();
        let mut out = vec![m.mirror()];
        for _ in 0..3 {
            // Fisher–Yates permutation of edges + random dart swaps
            let mut perm: Vec<usize> = (0..e).collect();
            for i in (1..e).rev() {
                perm.swap(i, rng.next(i + 1));
            }
            let flip: Vec<bool> = (0..e).map(|_| rng.next(2) == 1).collect();
            out.push(m.relabel(&perm, &flip));
        }
        let mut sub = m.clone();
        for _ in 0..3 {
            let d = rng.next(sub.n_darts);
            sub = sub.subdivide(d);
        }
        out.push(sub.clone());
        // the composite: subdivide, then relabel, then mirror
        let e2 = sub.edge_count();
        let mut perm: Vec<usize> = (0..e2).collect();
        for i in (1..e2).rev() {
            perm.swap(i, rng.next(i + 1));
        }
        let flip: Vec<bool> = (0..e2).map(|_| rng.next(2) == 1).collect();
        out.push(sub.relabel(&perm, &flip).mirror());
        out
    }

    #[test]
    fn reading_invariance_holds_for_the_presentation_class() {
        // The 𝔽₀ floor of READING-INVARIANCE-TARGET: for every reading generated by
        // relabeling, mirror and subdivision, the essential invariants are unchanged.
        let subjects: Vec<CombinatorialMap> = vec![
            cycle(3),
            cycle(6),
            two_triangles(),
            CombinatorialMap::path(2),
            cycle(3).disjoint_union(&cycle(3)).connect(0, 6),
        ];
        for (i, m) in subjects.iter().enumerate() {
            let base = m.essential_invariants();
            for (j, reading) in f0_battery(m, 0xA11CE + i as u64).iter().enumerate() {
                assert!(
                    reading.validate().is_ok(),
                    "subject {i}, reading {j}: valid"
                );
                assert_eq!(
                    reading.essential_invariants(),
                    base,
                    "subject {i}, reading {j}: it must not matter how you look at it"
                );
            }
        }
    }
