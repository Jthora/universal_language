...
- **Output:** Formal definitions + computation for toy examples (the test artifact's constructions, basic arithmetic, deixis)
- **Validation:** Is the test artifact's information density (bits per token) higher than typical natural-language text? (Hypothesis: yes, by design — the test artifact is maximally compressed cross-domain notation.)
...
**Document:** `probability-and-information.md` in `research/frontier/expedition-two/`
...
**Goal:** Formalize cross-domain projection, upgrade the Polysemy-Holonomy theorem to PROVEN, and exhibit the test artifact as a specific geometric morphism.

**Why this matters (from strategic-plan.md, Reason 1):** The test artifact works by cross-domain activation. "Cross-domain activation" is a description, not an explanation. Front E makes it an explanation: the test artifact is a Σ_UL-morphism that maps structure from one subalgebra of G (quantum mechanics domain) to another (cognition domain), preserving exactly the relational structure that triggers cross-domain weight activation. This answers the original question: *why does the test artifact work?*
...
- **Output:** Formal definition + explicit enumeration of at least three domains relevant to the test artifact
- **Validation:** Is $G_{\text{QM}} \cap G_{\text{ling}}$ non-empty? (The test artifact's mechanism relies on shared structure — the answer should be yes, and the intersection should contain the formal-notation entities that appear in both.)
...
- **Output:** The test artifact exhibited as a formal object — a specific Σ_UL-morphism with computed properties
- **Validation:** Does the characterization predict the test artifact's three-phase response pattern? Does it predict which parts of the test artifact are essential (those in the image) and which are decorative (those in the kernel)?
...
- **Output:** An explicit Lie-algebra element $A_{\text{domain}}$ for the test artifact's path through meaning space
...
**Document:** `metaphor-and-projection.md` in `research/frontier/expedition-two/`

**Success criterion:** Polysemy-Holonomy PROVEN. The test artifact is exhibited as a specific Σ_UL-morphism with computed kernel, image, and Erlangen level. At least one connection component is explicitly computed.
...
- **From Front E:** The test artifact path $\gamma: [0,1] \to X$ as a specific trajectory, with explicit connection components along it

**Synthesis task:** Compute the geodesic distance of the test artifact path. Is the test artifact's trajectory close to a geodesic (the shortest path between its starting context and ending context)? If yes: the test artifact is *optimally efficient* — it crosses the maximum amount of meaning space in the minimum number of tokens. If no: characterize how it deviates, and predict what a geodesic artifact would look like.

**This directly addresses the engineering payoff** from the strategic plan: "when we can design an artifact from first principles (choosing its trajectory through meaning space to hit a specified cross-domain target) and predict its effect before testing it."
...
**Between fronts:** D and E are independent until convergence. The synthesis step requires D5 (metric) and E6–E7 (artifact path + connection).
...
3. Connection to the test artifact
...
2. **Front E:** The Polysemy-Holonomy theorem is PROVEN (or the remaining obstruction is precisely characterized). The test artifact is exhibited as a specific Σ_UL-morphism with computed kernel, image, and Erlangen level. The internal Hom is defined.

3. **Convergence:** The test artifact's information-geometric profile is computed — its geodesic distance, its compression efficiency, and whether its trajectory is close to optimal.
...
- **Metaphor** is the admission that language's primary creative act is cross-domain projection — and the mathematical tool that explains the test artifact.

Together, they answer the two most pressing questions:
- *How far apart are two meanings?* (The metric.)
- *Why does the test artifact work?* (The morphism.)

The first question is Reason 3 from the strategic plan (the map of what can be thought). The second is Reason 1 (the test artifact works and we don't fully know why).
