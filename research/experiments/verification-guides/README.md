...
The original experimental protocol (`research/frontier/causal-efficacy-protocol.md`) requires API keys for OpenAI, Anthropic, or Google. This is a barrier that excludes most potential replicators. But the core experiment is simple:
...
All approaches use:
- The **same task prompts** (from `research/experiments/prompts/task-prompts.md`)
- The **same primer text** (from `research/experiments/test-artifacts/original/primer.txt`)
- The **same control texts** (from `research/experiments/test-artifacts/controls/`)
- The **same scoring rubrics** (simplified version in `simplified-scoring.md`, full version in `research/experiments/scoring/`)
